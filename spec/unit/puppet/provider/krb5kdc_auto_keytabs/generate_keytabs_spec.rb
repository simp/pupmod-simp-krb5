...
Entry for principal host/#{test_host} with kvno 1, encryption type aes128-cts-hmac-sha1-96 added to keytab WRFILE:test.keytab.
  ...
  before :each do
    ...
    provider.class.stubs(:kadmin).with(regexp_matches(/list_principals/)).returns(list_principals)
    provider.class.stubs(:kadmin).with(regexp_matches(/get_principal/)).returns(get_principal)
    provider.class.stubs(:kadmin).with(regexp_matches(/add_principal/)).returns(add_principal)
    provider.class.stubs(:kadmin).with(regexp_matches(/ktadd/)).returns(add_keytab)
  ...
  context 'generating keytabs' do
    it 'should not have any errors' do
      provider.class.expects(:kadmin).with(regexp_matches(%r(ktadd.*.+/#{test_host}@#{test_realm})))
      provider.class.expects(:kadmin).with(regexp_matches(%r(add_principal.*.+/#{test_host}@#{test_realm}))).never
  ...
  context 'with a matching kvno file' do
    it 'should not have any errors' do
      ...
      provider.class.expects(:kadmin).with(regexp_matches(%r(ktadd.*.+/#{test_host}@#{test_realm}))).never
      provider.class.expects(:kadmin).with(regexp_matches(%r(add_principal.*.+/#{test_host}@#{test_realm}))).never
  ...
  context 'with a non-matching kvno file' do
    it 'should not have any errors' do
      ...
      provider.class.expects(:kadmin).with(regexp_matches(%r(ktadd.*.+/#{test_host}@#{test_realm}))).once
      provider.class.expects(:kadmin).with(regexp_matches(%r(add_principal.*.+/#{test_host}@#{test_realm}))).never
...
puppet_auto/admin@#{test_realm}
    ...
    it 'should not have any errors' do
      provider.class.expects(:kadmin).with(regexp_matches(%r(ktadd.*.+/#{test_host}@#{test_realm}))).once
      provider.class.expects(:kadmin).with(regexp_matches(%r(add_principal.*.+/#{test_host}@#{test_realm}))).once
  ...
  context 'generating keytabs for all known principals' do
    ...
    it 'should not have any errors' do
      ...
      provider.class.expects(:kadmin).with(regexp_matches(%r(ktadd.*.+/#{test_host}@#{test_realm}))).twice
      provider.class.expects(:kadmin).with(regexp_matches(%r(add_principal.*.+/#{test_host}@#{test_realm}))).never
